...
//    filenamePLY = QString("%1.%2.ply").arg(YaLuxGlobal.tempCounter++).arg(objMatName);
    ...
    QTemporaryFile *plyOut = new QTemporaryFile(filenamePLY);

    // add the ply tempobject to the garbagecollection list
    YaLuxGlobal.PLYgarbageCollectionList.append(plyOut);
    
    plyOut->open();
    plyOut->write("ply\n");
    plyOut->write("format binary_little_endian 1.0\n");
    plyOut->write("comment PLY generated by yaluxplug\n");
    plyOut->write( QString("element vertex %1\n").arg(numVerts).toAscii() );
